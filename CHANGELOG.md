...
### Fixed
- **Reserved-name files can now actually be deleted** (script `Remove-ReservedFiles.ps1`): `Test-ValidPath` checked existence with plain Win32 semantics, so reserved device names (`nul`, `con`, `aux`, ...) resolved to their devices and every deletion failed with "Path does not exist". Existence is now checked via the `\\?\` extended path prefix.
- **`-UseRecycleBin` now truly recycles reserved-name files** (script and module): `FileSystem.DeleteFile` rejects `\\?\` paths and plain reserved-name paths resolve to the device, so recycling silently fell through to permanent deletion. Files are now renamed to a normal name (via `\\?\`) and then recycled.
- Post-deletion "still exists" verification now uses the `\\?\` prefix so reserved files are checked correctly.

### Added
- Pester regression tests: module `-UseRecycleBin` recycling, plus end-to-end script integration tests (delete and recycle) that reproduce the original failures.
...
[1.2.1]: https://github.com/RicherTunes/windows-reserved-file-cleaner/releases/tag/v1.2.1
